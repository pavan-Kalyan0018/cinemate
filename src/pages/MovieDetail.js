...
export const MovieDetail = ({title}) => {
  ...
  const pageTitle = useTitle({title: movie.title || "Loading..."});

  useEffect(() => {
    async function fetchMovie() {
      const response = await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=88252cfb90e36d825f856294fbca1e68`);
  ...
  useEffect(() => {
    document.title = movie.title ? `${movie.title} - Cinemate` : "Loading... - Cinemate";
  }, [movie.title]);
  return (
    <main>
      ...
      <div className='max-w-sm'>
        ...
        <div className='max-w-2xl text-gray-700 text-lg dark:text-white'>
          ...
          <div className="flex items-center">
              ...
              "w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                <title>Rating star</title><path d=
